...
class AgentQNetwork(nn.Module):
    ...
    def __init__(self, num_actions=21, input_channels=5):
        ...
        nn.init.kaiming_normal_(self.fc[-1].weight, mode='fan_in')
...
class QMIX:
    ...
    def __init__(
        ...
        self.scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optimizer, T_0=80, T_mult=2, eta_min=1e-6,
        )
        self.scheduler1 = optim.lr_scheduler.StepLR(
            self.optimizer, step_size=80, gamma=0.8
        )
    ...
    def update(self, batch: Dict[str, np.ndarray], ep: int) -> float:
        ...
        obs = torch.tensor(batch['obs'], dtype=torch.float32).to(device)            # (B, T, N, H, W, C)
        actions = torch.tensor(batch['actions'], dtype=torch.int64).to(device)        # (B, T, N)
        rewards = torch.tensor(batch['rewards'], dtype=torch.float32).to(device)     # (B, T,)
        next_obs = torch.tensor(batch['next_obs'], dtype=torch.float32).to(device)   # (B, T, N, H, W, C)
        state = torch.tensor(batch['state'], dtype=torch.float32).to(device)         # (B, T, H_s, W_s, C_s)
        next_state = torch.tensor(batch['next_state'], dtype=torch.float32).to(device)  # Same shape as state
        dones = torch.tensor(batch['dones'], dtype=torch.int64).to(device)            # (B, T,)
        ...
        obs_sb = obs.permute(0, 1, 4, 2, 3).contiguous()        # Shape: (sb_size, N, C, H, W)
        next_obs_sb = next_obs.permute(0, 1, 4, 2, 3).contiguous()
        ...
        actions_sb_flat = actions.view(-1).contiguous()                     # Shape: (sb_size * N)
    ...
    # Compute Double Q-learning targets
        ...
        state_sb = state.permute(0, 3, 1, 2).contiguous()  # Shape: (sb_size, C_s, H_s, W_s)
        next_state_sb = next_state.permute(0, 3, 1, 2).contiguous()
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            with torch.no_grad():
                ...
                targets = rewards + self.gamma * (1 - dones) * target_q_tot  # Shape: (sb_size, 1)
        ...
        # Free up GPU memory
        del obs_sb,  next_obs_sb, state_sb, next_state_sb 
...
if __name__ == "__main__":
    ...
    loss = nn.SmoothL1Loss()(mn(agent_qs, state), torch.randn(1, 1))
    loss.backward()