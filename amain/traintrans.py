from utils.qmixtrans import QMIX
from utils.rbtrans import ReplayBuffer, PrioritizedReplayBuffer
import gc
...
import os
import wandb
from utils.processtrans import process_batch, Normalizer 
import random
import numpy as np
import torch
import time
...
def train(config):
    ...
    rb = ReplayBuffer(memory_size=500, field_names=field_names)
    ...
    for ep in range(num_episodes):
        ...
        while not terminated:
            ...
            if len(rb) >= batch_size and ((count % 100) == 0) and ep >= 1   :
        ...
        #         qmix_red.update_target_soft(config.tau)
                ...
                # rb.update_priorities(ids, loss_blue)
