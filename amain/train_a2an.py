from amain.utils.qmix_a2an import QMIX
...
def train(config):
    ...
    env = battle_v4.parallel_env(map_size=45, minimap_mode=False, step_reward=-0.08,
            dead_penalty=-0.16, attack_penalty=-0.16, attack_opponent_reward=0.64, max_cycles=320, 
    ...
    qmix_red = QMIX(num_agents=81, agent_ids=red_agents, agent_ids1=blue_agents, state_shape=(5, 45, 45), device=device, lr=learning_rate, gamma=gamma)
    # qmix_blue.agent_q_network.load_state_dict(torch.load("/home284/284-home/UET/RL-final-UET/RL-final-project-AIT-3007/qmix_blue_ep599.pth"))
    # qmix_red.agent_q_network.load_state_dict(torch.load("/home284/284-home/UET/RL-final-UET/RL-final-project-AIT-3007/qmix_red_ep599.pth"))
    ...
    for ep in range(num_episodes):
        ...
        while not terminated:
            ...
            done_blue = np.all([dones[a] for a in blue_agents])
            done_red = np.all([dones[a] for a in red_agents])
