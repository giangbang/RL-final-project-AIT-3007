...
from process1 import process_episodes, collate_episodes
import wandb
import threading
...
def train(config):
    ...
    qmix_blue = QMIX(num_agents=81, state_shape=(45, 45, 5), device=device, lr=lr, gamma=gamma)
    qmix_red = QMIX(num_agents=81, state_shape=(45, 45, 5), device=device, lr=lr, gamma=gamma)

    buffer_size = 1
    tempdir = tempfile.TemporaryDirectory()
    replay_buffer = TensorDictReplayBuffer(
        storage=LazyMemmapStorage(100, scratch_dir=tempdir.name),
    ...
    for ep in range(num_episodes):
        ...
        while not termination: 
            ...
            transition = {
                ...
                "a_b": torch.from_numpy(action_array_blue),
                "a_r": torch.from_numpy(action_array_red),
                "r_b": torch.tensor(reward_array_blue, dtype=torch.float32),
                "r_r": torch.tensor(reward_array_red, dtype=torch.float32),
                "d_b": torch.tensor(done_blue, dtype=torch.uint8),
                "d_r": torch.tensor(done_red, dtype=torch.uint8),
        ...
        if len(replay_buffer) >= batch_size and ep >= 60:
        ...
        if ((ep+1) % 50) == 0:
                parent_dir = "./drive/"
                save_path_blue = "./drive/qmix_blue_ep{}.pth".format(ep)
                save_path_red = "./drive/qmix_red_ep{}.pth".format(ep)
                ...
                torch.save(qmix_blue.mixing_network.state_dict(), "./drive/mn_blue_ep{}.pth".format(ep))
                torch.save(qmix_red.mixing_network.state_dict(), "./drive/mn_red_ep{}.pth".format(ep))
        ...
        if (save_time_seconds - elapsed_time) <= 100:
            ...
            qmix_blue.update_target_hard()
            qmix_red.update_target_hard()
            torch.save(qmix_blue.agent_q_network.state_dict(), './drive/aqn_model_bafter_2_hours.pth')
            torch.save(qmix_red.agent_q_network.state_dict(), './drive/aqn_model_rafter_2_hours.pth')
            torch.save(qmix_blue.mixing_network.state_dict(), './drive/mn_model_btarget_after_2_hours.pth')
            torch.save(qmix_red.mixing_network.state_dict(), './drive/mn_model_rtarget_after_2_hours.pth')
