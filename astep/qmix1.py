...
class SharedActorNetwork(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(batch_size, 128).to(device)
...
class QNetwork(nn.Module):
    def __init__(self, observation_shape, action_shape=21):
        ...
        # Calculate the size after CNN layers
        self._create_fc(observation_shape)
        self.fc1 = nn.Linear(self.cnn_output_dim+8, 64)
        self.fc2 = nn.Sequential(
...
class MixingNetwork(nn.Module):
    ...
    def forward(self, agent_qs, state):
        ...
        hidden = F.relu(torch.bmm(agent_qs.unsqueeze(1), w1) + b1)  # (batch_size*transiition, 1, embed_dim)
...
class QMIX:
    ...
    def select_actions(self, obs: torch.Tensor, prev_action: None, 
        ...
        B, H, W, C = obs.shape
        obs = obs.view(B,C,H,W).contiguous()
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> float:
        ...
        for b in range(B):
            for i in range(T-1): # Transitions
                ...
                if i != 0:
                    prev_actions_sb = actions[:,i-1]  # Shape: (B, N)
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                with torch.no_grad():
                    ...
                    targets = rewards_sb + self.gamma * (1 - done_sb) * target_q_tot  # Shape: (sb_size, 1)

                masked_td = (q_tot - targets)*(1-done_sb)  # Shape: (sb_size, 1)
                done_sum = torch.sum(1-done_sb, dtype=torch.float32)
                if done_sum:
                    loss += (masked_td ** 2).sum()/done_sum  # Shape: (1,)
                    entropy += -torch.sum(F.softmax(q_values, dim=1) * F.log_softmax(q_values, dim=1), dim=1).mean()/T*(1-done_sb) # (sb_size * N)
                    loss += 0.03*entropy
