...
class QMIX:
    ...
    def update(self, batch):
        ...
        obs = torch.from_numpy(batch['obs'], device=device))
        actions = torch.tensor(batch['actions'], dtype=torch.long, deivce=device)
...
if __name__ == "__main__":
    ...
    for ep in range(2):
        ...
        # joint_obs_blue = {a: obs[a] for a in blue_agents}
        # joint_obs_red = {a: obs[a] for a in red_agents}
        ...
        while not terminated:
            ...
            rb.save_to_memory(
                ...
                rewards, # dict of rewards of all agents
            ...
            if len(rb) > batch_size:
                ...
                for field in field_names:
                    if field == "state" or field == "next_state" or field == "dones":

                        batch_state = np.stack([batch[field] for _ in range(batch_size)], axis=0)
                        batch_blue[field] = batch_state
                        batch_red[field] = batch_state
                        continue
                    for i in range(len(batch[field])):
                        keys = list(batch[field][i].keys())
                        blue_agents = [agent for agent in keys if agent.startswith("blue")]
                        red_agents = [agent for agent in keys if agent.startswith("red")]
                        # for each, blue and red agents, stack their fields, so that they can have format (B, num_agents, ...) 
                        if len(blue_agents) > 0:
                            if field == "obs" or field == "next_obs":
                                batch_blue[field].append(np.stack([batch[field][i][a] for a in blue_agents], axis=1))
                            else:
                                batch_blue[field].append(np.stack([[batch[field][i][a] for a in blue_agents]], axis=1))
                        if len(red_agents) > 0:
                            if field == "obs" or field == "next_obs":
                                batch_red[field].append(np.stack([batch[field][i][a] for a in red_agents], axis=1))
                            else:
                                batch_red[field].append(np.stack([[batch[field][i][a] for a in red_agents]], axis=1))
                            # batch_red[field].append(np.stack([batch[field][i][a] for a in red_agents], axis=1))


                # batch_blue is a dictionary with keys: obs, actions, rewards, next_obs, dones, state, next_state
                # batch_red is a dictionary with keys: obs, actions, rewards, next_obs, dones, state, next_state
                # batch_blue['obs']: list(N_blue, 13,13,5)             
                for field in field_names:
                    if field == "state" or field == "next_state" or field == "dones":
                        continue
                    if len(batch_blue[field]) > 0:
                        batch_blue[field] = np.stack(batch_blue[field], axis=0)
                    if len(batch_red[field]) > 0:
                        batch_red[field] = np.stack(batch_red[field], axis=0)
                

                # Reformat batch to match QMIX training steps
                # This involves stacking all agent's data into appropriate shapes.
                # Due to complexity, assume batch is already pre-processed into the correct format:
